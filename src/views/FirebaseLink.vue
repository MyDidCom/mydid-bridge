<template>
  <div class="flex flex-col justify-center items-center h-screen">
    ...
    <div class="bg-white rounded-xl p-5">
      ...
      <p
        v-if="!loading && dynamicLink"
        class="text-center text-xl font-bold mt-6"
      >
        Scan with myDid application
      </p>
      <p
        v-if="!loading && !dynamicLink"
        class="text-center text-xl font-bold text-red-500 mt-6"
      >
        Bad data in URL
      </p>
    </div>
...
export default {
  setup() {
    ...
    const urlData = route.query.uri;
    if (urlData) {
      console.log(urlData);
      loading.value = true;
      api
        .getDynamicLink(urlData)
        .then((res) => {
          dynamicLink.value = res.data.dynamicLink;
          loading.value = false;
        })
        .catch((e) => {
          loading.value = false;
    ...
    return { dynamicLink, loading, isMobile };
  },
};
</script>
